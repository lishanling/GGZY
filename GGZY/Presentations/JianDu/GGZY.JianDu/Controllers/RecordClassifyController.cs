...
{
    public partial class JdRecordClassifyController
    {
        /// <summary>
        /// 保存或提交
        /// </summary>
        ...
        /// <returns></returns>
        //[ValidateInput(false)]
        public ActionResult SaveOrSubmit(RECORD_REVIEW review, RECORD_CLASSIFY record_biddoc,
            List<UploadResData_ReviewRequest> attachments)
        {
            ...
            if (null != record_biddoc && record_biddoc.RECORD_ID.HasValue && record_biddoc.RECORD_ID > 0)
            {
                ...
                if(record_review.REVIEW_STATUS == "5" || record_review.REVIEW_STATUS == "6")
                {
                    record_review.IS_SIGN = 1;
                    record_review.REVIEW_STATUS = "4";
                }
                review = record_review;
            }

            var data = Jd.RecordClassifyService.SaveOrSubmit(tender_project, review, record_biddoc, attachments);
            return JResult(data);
        }
    }
}