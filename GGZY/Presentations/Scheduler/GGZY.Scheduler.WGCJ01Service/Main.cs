...
namespace GGZY.Scheduler.WGCJ01Service
{
    /// <summary>
    /// 违规行为拆解
    /// 依法招标项目中标段包个数有2个及以上，“在线审批平台”中的项目投资规模高于400万元，
    /// 比对“电子公共服务平台”发现其下存在非依法招标的标段包且招标金额低于400万元的项目，则提示
    /// </summary>
    public class Main : GGZY.Services.Base.BaseServiceT<Dos.ORM.Entity>, IJob
    {
        protected string point_code = "WG0301";
        public Task Execute(IJobExecutionContext context)
        {
            return Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    GGZY.Core.Log.Logger.Info("违规拆解分析 " + point_code + " 任务开始执行--------------");
                    ...
                    sb.AppendLine("               ROW_NUMBER() OVER(PARTITION BY(TPC.PROJECT_CODE) ORDER BY TPC.TENDER_PROJECT_CODE DESC) RN");
                    ...
                    sb.AppendLine("         INNER JOIN (SELECT COUNT(1) AS COUNT, TENDER_PROJECT_CODE");
                    sb.AppendLine("                      FROM SECTION");
                    sb.AppendLine("                     GROUP BY TENDER_PROJECT_CODE");
                    sb.AppendLine("                    HAVING COUNT(1) > 1 AND MIN(CASE WHEN PRICE_UNIT='1' THEN CONTRACT_RECKON_PRICE * 10000 ELSE CONTRACT_RECKON_PRICE END) < 4000000) S");
                    sb.AppendLine("            ON S.TENDER_PROJECT_CODE = TPC.TENDER_PROJECT_CODE");
                    sb.AppendLine("         WHERE JDAFC.TOTAL_INVEST > 400");
                    sb.AppendLine("           AND TPC.IS_TO_LAW = 1) T");
                    ...
                    projects.ForEach(e => {
                        Logger.Info("开始违规拆解分析 - "+point_code+" 招标项目编号：" + e.TENDER_PROJECT_CODE + ",招标项目名称：" + e.TENDER_PROJECT_NAME);
                        try
                        {
                            ...
                            var result = new
                            {
                                TOTAL_INVEST = e.TOTAL_INVEST + " 万元",
                                SECTION_COUNT = e.COUNT + " 个",
                                SECTION = datas
                            };
                            ...
                            Logger.Info("违规拆解分析 - " + point_code + " 完成 招标项目编号：" + e.TENDER_PROJECT_CODE + ",招标项目名称：" + e.TENDER_PROJECT_NAME);
                        }
                        catch(Exception exx)
                        {
                            Logger.Info("违规拆解分析 - " + point_code + " 异常 招标项目编号：" + e.TENDER_PROJECT_CODE + ",招标项目名称：" 
                                + e.TENDER_PROJECT_NAME +"，异常信息："+exx.Message,exx);
                        }
                    });
                    GGZY.Core.Log.Logger.Info("违规拆解分析 - "+ point_code + " 任务执行完毕--------------");
                }
                catch (Exception ex)
                {
                    Logger.Error("违规拆解分析 - " + point_code + " 分析发生异常:" + ex.Message, ex);
                }
            });
        }
        public class RESULT
