...
namespace GGZY.Scheduler.Clue02Service
{
    public class MainService : GGZY.Services.Base.BaseServiceT<BD_VIOLATION_POINT_DETAIL>, IJob
    {
        protected string point_code = "WG080201";
        public Task Execute(IJobExecutionContext context)
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                string sqlBidder = "select distinct WIN_BIDDER_NAME from V_TENDERAGENCY_BIDDER";
                ...
                GGZY.Core.Log.Logger.Info("开始查询多次在同一代理中标的投标人名单--------------");
                for (int i = 1; i <= maxPage; i++)
                {
                    try
                    {
                        GGZY.Core.Log.Logger.Info("开始查询多次在同一代理中标的投标人名单 第" + (pageSize * (i - 1) + 1) + " 至" +
                           pageSize * i + " 条记录 --------------");
                        var bidders = FromSql(sqlBidder).Page(pageSize,i).ToPageTable(out totalCount, "WIN_BIDDER_NAME"); ;
                        GGZY.Core.Log.Logger.Info("完成查询多次在同一代理中标的投标人名单 第" + (pageSize * (i - 1) + 1) + " 至" +
                           pageSize * i + " 条记录 --------------");
                        var m = 1;
                        bidders.AsEnumerable().Select(t => t["WIN_BIDDER_NAME"].ToString()).ToList<string>().ForEach(e => {
                            try
                            {
                                GGZY.Core.Log.Logger.Info("开始分析 "+ (pageSize * (i - 1) + m) + ":"+ e + " 多次中标情况--------------");
                                var results = FromSql("SELECT * FROM V_TENDERAGENCY_BIDDER_DETAIL WHERE WIN_BIDDER_NAME='" + e + "'").ToList<RESULT>();
                                if (results.Any())
                                {
                                    ...
                                    if (!String.IsNullOrEmpty(entity_id))
                                    {
                                        ...
                                        GGZY.Core.Log.Logger.Info((pageSize * (i - 1) + m) + "：" + e + " 分析结果数据更新完成--------------");
                                        return;
                                    }
                                    ...
                                    GGZY.Core.Log.Logger.Info((pageSize * (i - 1) + m) + "：" + e + " 分析结果数据新增完成--------------");
                                }
                            }
                            catch (Exception exd)
                            {
                                Logger.Error("处理多次在同一代理-中标人的投标人名单发生异常:" + exd.Message, exd);
                            }
                            finally
                            {
                                m++;
                    ...
                    {
                        Logger.Error("分析多次在同一代理中标的投标人名单发生异常:" + ex.Message, ex);
                    }
                }
                GGZY.Core.Log.Logger.Info("多次在同一代理中标的投标人名单分析完成--------------");
