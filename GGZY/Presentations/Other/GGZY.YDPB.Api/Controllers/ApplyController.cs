    ...
    public class ApplyController : BaseApiController
    {
       ...
       [HttpPost]
        public GeneralResult Index([FromBody]RequestModel dic)
        {
            ...
            if (apply.SEAT_NUMBER == null || !apply.SEAT_NUMBER.Any())
            {
                result.SetFail("主会场评标席位不能为空");
                return result;
            }

            if (apply.SUB_DETAIL == null || ! apply.SUB_DETAIL.Any())
            {
                result.SetFail("分会场申请信息不能为空");
                return result;
            }
            ...
            return Jd.YdpbEvalApplyService.I_Add(entity,details, seat,apply.SEAT_NUMBER);
        }
    }
}