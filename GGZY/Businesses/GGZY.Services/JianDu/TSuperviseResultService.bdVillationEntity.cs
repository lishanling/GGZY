...
namespace GGZY.Services.JianDu
{
    public partial class TSuperviseResultService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public GeneralResult BdViolationEntitySupervise(TSuperviseModel model)
        {
            var r = new GeneralResult();
            model.M_ID = Guid.NewGuid().ToString();
            model.TIME = DateTime.Now;
            model.STATUS = 0;
            model.ENTITY_TYPE_NAME = nameof(GGZYFW.DbEntity.BD_VIOLATION_ENTITY);
            ...
            var superviseItem = Fw.BdViolationEntityService
                .FirstOrNull(GGZYFW.DbEntity.BD_VIOLATION_ENTITY._.ID == model.ENTITY_ID);
            model.TENDER_PROJECT_CODE = superviseItem?.ENTITY_CODE;
            model.TENDER_PROJECT_NAME = superviseItem?.ENTITY_NAME;
            var tenderProjectType = "";
            switch (superviseItem?.ENTITY_TYPE)
            {
                case 1:tenderProjectType = "招标人"; break;
                case 2: tenderProjectType = "代理机构"; break;
                case 3: tenderProjectType = "投标人"; break;
            }
            model.TENDER_PROJECT_TYPE = tenderProjectType;
            model.REGION_CODE = superviseItem?.AREA_CODE;
            model.ENTITY_NAME = superviseItem?.ENTITY_NAME;
