...
namespace GGZY.Services.JianDu
{
    public partial class ProjectService : BaseServiceT<PROJECT>
    {
       ...
       var pageList =PageList(condition,search, PROJECT._.M_ID.Desc).ToBootstrapTableList(count);
       	...
       	protected WhereClipBuilder ConditionBuilder(PROJECT model)
	{
		WhereClipBuilder condition = new WhereClipBuilder();
				...
				{
				  condition.And(PROJECT._.CONSTRUCTION_ID.Contain(model.CONSTRUCTION_ID));
				}
				if(model.M_ID!=null)
				{
				...
				{
				  condition.And(PROJECT._.REGION_CITY.Contain(model.REGION_CITY));
				}
				if(!string.IsNullOrWhiteSpace(model.REGION_COUNTY))
				{
				  condition.And(PROJECT._.REGION_COUNTY.Contain(model.REGION_COUNTY));
				}
		return condition;
	}
        #endregion
