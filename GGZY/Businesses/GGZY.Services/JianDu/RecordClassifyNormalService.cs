...
{
    public partial class RecordClassifyNormalService 
    {
        /// <summary>
        /// 澄清修改
        /// </summary>
		...
		public GeneralResult Detail(int record_id, Func<string, string> urlBuilder)
        {
            ...
            if (null == entity)
            {
                r.SetFail("澄清修改备案记录不存在.未找到 RECORD_CLASSIFY_NORMAL 信息");
                return r;
            }

            var conditionInstance = new WhereClipBuilder();
            conditionInstance.And(REVIEW_INSTANCE._.BA_ID == record_id);
            //conditionInstance.And(REVIEW_INSTANCE._.IS_END != 0);
            //conditionInstance.And(REVIEW_INSTANCE._.MAIN_ID.NotIn(4, 5));
            ...
            var data = null == entity?new Dictionary<string,object>():entity.ToBindDicDictionary();
        ...
        /// <returns></returns>
        public GeneralResult SaveOrSubmit(TENDER_PROJECT tender_project, RECORD_REVIEW review, RECORD_CLASSIFY_NORMAL biddoc, List<UploadResData_ReviewRequest> attachments)
        {
            ...
            using (var tran = db.BeginTransaction())
            {
                try
                {
                    ...
                    if (!isUpdate)
                    {
                        ...
                        tran.Insert<RECORD_CLASSIFY_NORMAL>(biddoc);
                    }
                    else
                    {
                        var model = FirstOrNull(RECORD_CLASSIFY_NORMAL._.RECORD_ID == review.ID);
                        if (null == model)
                        {
                            tran.Rollback();
                            r.SetFail("澄清修改备案 RECORD_BIDDOC_NORMAL 记录不存在.");
                            return r;
                        }
                        model.Attach();
                        ...
                        tran.Update<RECORD_CLASSIFY_NORMAL>(model);
                    }
                    var data = new
                    {
                ...
                {
                    tran.Rollback();
                    LoggerR.Error("澄清修改备案提交失败:" + ex.Message, ex);
                    r.SetException("澄清修改备案提交失败:" + ex.Message, ex);
                    return r;
                }
            }
        }
