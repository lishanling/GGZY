...
namespace GGZY.Services.JianDu
{
    public partial class TenderProjectDelService : BaseServiceT<TENDER_PROJECT_DEL>
    {
       ...
       var pageList =PageList(condition,search, TENDER_PROJECT_DEL._.ID.Desc).ToBootstrapTableList(count);
       	...
       	protected WhereClipBuilder ConditionBuilder(TENDER_PROJECT_DEL model)
	{
		WhereClipBuilder condition = new WhereClipBuilder();
				...
				{
				  condition.And(TENDER_PROJECT_DEL._.RECORD_DEPID.Contain(model.RECORD_DEPID));
				}
				if(!string.IsNullOrWhiteSpace(model.CONSTRUCTION_ID))
				{
				  condition.And(TENDER_PROJECT_DEL._.CONSTRUCTION_ID.Contain(model.CONSTRUCTION_ID));
				}
				...
				{
				  condition.And(TENDER_PROJECT_DEL._.M_CREATE_TM==(model.M_CREATE_TM));
				}
				if(model.ID!=null)
				{
				...
				{
				  condition.And(TENDER_PROJECT_DEL._.TENDERER_CONTRACT.Contain(model.TENDERER_CONTRACT));
				}
				if(!string.IsNullOrWhiteSpace(model.TENDERER_PHONE))
				{
				  condition.And(TENDER_PROJECT_DEL._.TENDERER_PHONE.Contain(model.TENDERER_PHONE));
				}
				if(!string.IsNullOrWhiteSpace(model.TENDER_AGENT_CONTRACT))
				{
				  condition.And(TENDER_PROJECT_DEL._.TENDER_AGENT_CONTRACT.Contain(model.TENDER_AGENT_CONTRACT));
				}
				if(!string.IsNullOrWhiteSpace(model.TENDER_AGENT_PHONE))
				{
				  condition.And(TENDER_PROJECT_DEL._.TENDER_AGENT_PHONE.Contain(model.TENDER_AGENT_PHONE));
				}
				if(!string.IsNullOrWhiteSpace(model.TENDER_DOCNAME))
				{
				  condition.And(TENDER_PROJECT_DEL._.TENDER_DOCNAME.Contain(model.TENDER_DOCNAME));
				}
				if(!string.IsNullOrWhiteSpace(model.RTYPE))
				{
				  condition.And(TENDER_PROJECT_DEL._.RTYPE.Contain(model.RTYPE));
				}
				if(!string.IsNullOrWhiteSpace(model.OTHER_DEPID))
				{
				  condition.And(TENDER_PROJECT_DEL._.OTHER_DEPID.Contain(model.OTHER_DEPID));
				}
		return condition;
	}
        #endregion
