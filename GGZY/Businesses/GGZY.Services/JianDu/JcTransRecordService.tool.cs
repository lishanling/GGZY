...
namespace GGZY.Services.JianDu
{
	public partial class JcTransRecordService : BaseServiceT<JC_TRANS_RECORD>
	{
		...
		public GeneralResult Edit(JC_TRANS_RECORD model)
		{
			...
			try
			{
				...
				if (viewResult.Success && viewResult.Data is JC_TRANS_RECORD entity)
				{
					...
					entity.TRANS_USER_PHONE = model.TRANS_USER_PHONE;
					entity.RECEIVER_UNIT_REGION = model.RECEIVER_UNIT_REGION;
					entity.TRANS_UNIT_REGION = model.TRANS_UNIT_REGION;
					entity.RECEIVER_UNIT_HY_TYPE = model.RECEIVER_UNIT_HY_TYPE;
					entity.TRANS_UNIT_HY_TYPE = model.TRANS_UNIT_HY_TYPE;
					//Update(entity);
		...
		protected WhereClipBuilder ConditionBuilder(JC_TRANS_RECORD model)
		{
			...
			if (!string.IsNullOrWhiteSpace(model.TRANS_USER_PHONE))
			{
				condition.And(JC_TRANS_RECORD._.TRANS_USER_PHONE.Contain(model.TRANS_USER_PHONE));
			}
			if (!string.IsNullOrWhiteSpace(model.RECEIVER_UNIT_REGION))
			{
				condition.And(JC_TRANS_RECORD._.RECEIVER_UNIT_REGION.Contain(model.RECEIVER_UNIT_REGION));
			}
			if (!string.IsNullOrWhiteSpace(model.TRANS_UNIT_REGION))
			{
				condition.And(JC_TRANS_RECORD._.TRANS_UNIT_REGION.Contain(model.TRANS_UNIT_REGION));
			}
			if (!string.IsNullOrWhiteSpace(model.RECEIVER_UNIT_HY_TYPE))
			{
				condition.And(JC_TRANS_RECORD._.RECEIVER_UNIT_HY_TYPE.Contain(model.RECEIVER_UNIT_HY_TYPE));
			}
			if (!string.IsNullOrWhiteSpace(model.TRANS_UNIT_HY_TYPE))
			{
				condition.And(JC_TRANS_RECORD._.TRANS_UNIT_HY_TYPE.Contain(model.TRANS_UNIT_HY_TYPE));
			}
			return condition;
		}
		#endregion
