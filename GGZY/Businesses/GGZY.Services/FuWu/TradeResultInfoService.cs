...
{
    public partial class TradeResultInfoService 
    {
        /// <summary>
        ...
        /// <returns></returns>
        public object JD_GYCQ_SW_TradeResultInfo(string projectCode)
        {
            var obj = FromSql(
                    @"SELECT A.PROJECT_NAME,A.PROJECT_CODE,A.EXCHANGE_TYPE,B.TEXT AS EXCHANGE_TYPE_TEXT,A.SELLER_NAME,A.SELLER_CODE,A.SELLER_TYPE,C.TEXT AS SELLER_TYPE_TEXT
                            ,A.SELLER_ROLE,A.PUB_SERVICE_PLAT_CODE,D.TEXT AS SELLER_ROLE_TEXT,A.OBJECT_EVALUATE_VALUE,A.TRADE_VALUE,A.PRICE_UNIT,E.TEXT AS PRICE_UNIT_TEXT,A.TRADE_DATE,A.PROJECT_URL
                            FROM TRADE_RESULT_INFO A 
                            LEFT JOIN SYS_DIC B ON A.EXCHANGE_TYPE =B.VALUE AND B.TYPE =23 AND B.VALUE5 ='国有产权（实物资产类）'
